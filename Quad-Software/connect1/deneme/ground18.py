...
SRT_LATENCY_MS = 60          # Jetson ile aynı tutmak mantıklı
...
META_STALE_SEC = 0.6         # meta gecikirse kutu “takılı kalmasın” diye kısa tut
...
meta_state = {
    "ts_recv": 0.0,
    "payload": {"ts":0.0, "infer_ms":0.0, "detections":[], "seq":0, "det_count":0, "trt_ok":False}
}
...
def draw_dets(frame, dets):
    ...
    for d in dets:
        ...
        cv2.putText(frame, "c{} {:.2f}".format(cls, conf),
                    (x1, max(20, y1-8)),
...
def draw_overlay(frame, tel, fps, meta_payload, meta_age):
    cv2.putText(frame, "FPS: {:.1f}".format(fps), (20, 30),
                cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255,255,255), 2)
    cv2.putText(frame, "Mode:{} Armed:{}".format(tel["mode"], tel["armed"]), (20, 60),
                cv2.FONT_HERSHEY_SIMPLEX, 0.6, (220,220,220), 1)

    cv2.putText(frame, "TRT:{} det:{} infer:{:.1f}ms meta_age:{:.2f}s".format(
        "OK" if meta_payload.get("trt_ok", False) else "NO",
        int(meta_payload.get("det_count", len(meta_payload.get("detections", [])))),
        float(meta_payload.get("infer_ms", 0.0)),
        float(meta_age)
    ), (20, 90), cv2.FONT_HERSHEY_SIMPLEX, 0.55, (200,200,200), 1)
    ...
    try:
        while True:
            ...
            with meta_lock:
                ...
                age = time.time() - float(meta_state["ts_recv"])

            # Meta tazeyse kutu çiz
            if age <= META_STALE_SEC:
                dets = payload.get("detections", [])
                if dets:
                    frame = draw_dets(frame, dets)

            draw_overlay(frame, tel, fps, payload, age)
