...
vision_jetson.py
────────────────
sender_jetson.py'in görüntü işleme + SRT stream katmanına
shared_data entegrasyonu eklenmiş hali.

Yaptığı işler:
  1. Kameradan frame yakala  (capture_loop)
  2. TensorRT ile inference  (infer_loop)
     - Bounding box'ları aynı frame'e çiz (senkron)
     - triangle → red_center, hexagon → blue_center olarak shared_data'ya yaz
  3. Annotated frame'i SRT ile PC'ye gönder  (stream_loop)
  4. Detection metadata'sını UDP ile PC'ye gönder  (meta_loop)
"""

from __future__ import print_function
import os, cv2, time, json, socket, threading
import numpy as np
from shared_data import shared, CLASS_TRIANGLE, CLASS_HEXAGON

# Sınıf isim haritası — sadece dict lookup, gecikme YOK
CLASS_NAMES = {
    CLASS_TRIANGLE: "triangle",
    CLASS_HEXAGON:  "hexagon",
}
...
# ═══════════════════════════════════════════════════════════════
#  PAYLAŞIMLI DURUM  (vision modülü içi)
# ═══════════════════════════════════════════════════════════════
running         = True
...
def build_camera_candidates():
    if USE_CSI_CAMERA:
        ...
        return [("usb_mjpeg", p1), ("usb_raw", p2)]
...
def draw_dets_on_frame(frame, dets, error_x, error_y):
    """
    Bounding box + hedef çizgisi çizer.
    triangle (kırmızı hedef) → mavi çizgi
    hexagon  (mavi hedef)    → kırmızı çizgi
    Eski vision.py'deki renk mantığı korundu.
    """
    cx = int(WIDTH  / 2) + error_x
    cy = int(HEIGHT / 2) + error_y
    ...
    for d in dets:
        ...
        if cls == CLASS_TRIANGLE:
            color = (255, 0, 0)   # mavi kutu (BGR) - kırmızı hedef
        elif cls == CLASS_HEXAGON:
            color = (0, 0, 255)   # kırmızı kutu - mavi hedef
        else:
            color = (0, 200, 255)

        cv2.rectangle(frame, (x1, y1), (x2, y2), color, 2)
        cv2.line(frame, (cx, cy), center, color, 2)
        name = CLASS_NAMES.get(cls, "c{}".format(cls))   # <-- bu satır
        lbl = "{} {:.2f}".format(name, conf)             # <-- ve bu
...
# ═══════════════════════════════════════════════════════════════
#  DETECTION → CENTER POINT ÇEVIRICI
# ═══════════════════════════════════════════════════════════════
def dets_to_centers(dets):
    """
    Detection listesinden blue_center ve red_center pixel koordinatlarını çıkarır.
    Birden fazla aynı sınıf varsa en yüksek confidence'lısı seçilir.
    Koordinatlar eski Hailo kodundaki gibi x-ekseninde aynalama yapılmaz
    (kamera yönüne göre değiştirmek gerekirse buradan ayarlayın).
    """
    blue_center = (None, None)
    red_center  = (None, None)
    best_blue_conf = -1
    best_red_conf  = -1
    ...
    for d in dets:
        ...
        if d["cls"] == CLASS_HEXAGON and conf > best_blue_conf:
            blue_center = (cx, cy)
            best_blue_conf = conf
        elif d["cls"] == CLASS_TRIANGLE and conf > best_red_conf:
            red_center = (cx, cy)
            best_red_conf = conf
...
# ═══════════════════════════════════════════════════════════════
#  TRT DETECTOR  (sender_jetson.py'den birebir alındı)
# ═══════════════════════════════════════════════════════════════
class TRTDetector(object):
    def __init__(self, engine_path):
        ...
        self.host_in = None; self.dev_in = None
        self.host_out = []; self.dev_out = []; self.out_bind_idxs = []
        for i in range(self.engine.num_bindings):
            ...
            if self.engine.binding_is_input(i):
                self.host_in = host_mem; self.dev_in = dev_mem
            else:
    ...
    def _decode_cn(self, out_cn, orig_w, orig_h, ratio, dw, dh, use_obj):
        ...
        if use_obj and score_mat.shape[0] >= 2:
            obj      = score_mat[0, :]; cls_scores = score_mat[1:, :]
        ...
        for j in idxs:
            ...
            x1 = (cx - w/2.0 - dw) / ratio; y1 = (cy - h/2.0 - dh) / ratio
            x2 = (cx + w/2.0 - dw) / ratio; y2 = (cy + h/2.0 - dh) / ratio
            x1, y1, x2, y2 = clip_box(x1, y1, x2, y2, orig_w, orig_h)
            boxes.append([x1,y1,x2,y2]); scores.append(float(conf[j]))
            clses.append(int(cls_ids[j]))
        keep = nms_xyxy(boxes, scores, IOU_THRES)
        return [{"cls": int(clses[i]), "conf": float(scores[i]),
    ...
    def decode_auto(self, out, orig_w, orig_h, ratio, dw, dh):
        if out.ndim == 3 and out.shape[0] == 1: out = out[0]
        out = np.squeeze(out)
        if not self._shape_logged:
            print("[TRT] output shape:", out.shape); self._shape_logged = True
        if out.ndim != 2: return []
        cands = []
        def add_cand(name, dets):
            if dets: cands.append((name, dets, len(dets),
                                   sum(d["conf"] for d in dets)))
...
def capture_loop():
    ...
    while running:
        ...
        if not ok or frame is None:
            ...
            cap = None; time.sleep(0.4); continue
...
def infer_loop():
    """
    Inference → center point hesapla → shared_data'ya yaz
    """
    ...
    while running:
        ...
        with state_lock:
            if latest_raw is None: continue
        ...
        # ── CENTER POINT → shared_data ──────────────────────────
        drone_data = shared.get_data()
        error_x    = int(drone_data[0]) if drone_data[0] == drone_data[0] else 0  # NaN guard
        error_y    = int(drone_data[1]) if drone_data[1] == drone_data[1] else 0

        blue_center, red_center = dets_to_centers(dets)
        shared.update_center(blue_center, red_center, False)
        shared.update_detections(dets, infer_ms)

        # ── BOUNDING BOX'LARI FRAME'E ÇİZ ──────────────────────
        if dets:
            frame = draw_dets_on_frame(frame, dets, error_x, error_y)
        frame = np.ascontiguousarray(frame)

        # ── ANNOTATED FRAME PAYLAŞ ──────────────────────────────
        with state_lock:
        ...
        # ── QUIT SİNYALİ ───────────────────────────────────────
        if drone_data[2]:
            print("[VISION] Quit sinyali alındı, durduruluyor.")
            running = False
            break

    try:
        if ctx: ctx.pop(); ctx.detach()
    except: pass


def stream_loop():   #special trial will be done -seyit
    global running
    wr = None; sent = 0; t_log = time.time()
    target_interval = 1.0 / FPS
    last_sent_seq   = -1
    ...
    while running:
        if wr is None:
            wr = open_writer()
            if wr is None: time.sleep(2.0); continue
        ...
        try:
            ...
            wr = None; time.sleep(0.3); continue

        now = time.time()
        if now - t_log >= LOG_EVERY_SEC:
            is_new = "yeni" if seq != last_sent_seq else "tekrar"
            print("[SRT] sent={} seq={} ({})".format(sent, seq, is_new))
            t_log = now; last_sent_seq = seq
...
def run_vision():
    """main.py'den thread olarak çağrılır."""
    global running
    print("[SYS] Vision modülü başlatılıyor...")
    ...
    if not opencv_has_gstreamer():
        print("[ERR] OpenCV GStreamer desteği yok!"); return

    threads = [
        threading.Thread(target=capture_loop, daemon=True, name="capture"),
        threading.Thread(target=infer_loop,   daemon=True, name="infer"),
        threading.Thread(target=stream_loop,  daemon=True, name="stream"),
        threading.Thread(target=meta_loop,    daemon=True, name="meta"),
    ]
    for t in threads: t.start()
    for t in threads: t.join()
    print("[SYS] Vision modülü bitti.")
