...
# -*- coding: utf-8 -*-
"""
Jetson Nano Sender (Python 3.6) - OPTIMIZED
- Kamera al (GStreamer / fallback V4L2)
- TensorRT engine ile tespit (inference)
- SRT (MPEG-TS/H264) video gönder (KUTUSUZ)
- UDP ile metadata gönder (detections)

Hedef:
- Jetson daha az ısınsın
- Gecikme düşsün
- Kutu çizimi PC tarafında yapılsın
"""
...
HEIGHT = 480
FPS = 30
BITRATE_KBPS = 2200          # biraz düşürdük -> encoder daha rahat, ısı biraz daha az
SRT_LATENCY_MS = 60          # 120->60: gecikme düşer (ağ çok kötü ise tekrar artırabilirsin)

# KUTU ÇİZMEYİ KAPATIYORUZ (PC çizecek)
...
INFER_EVERY_N = 2            # <<< en büyük ısı/fps kazancı: her framede inference yapma
DECODE_MODE = "noobj"        # sizde işe yarayan mod buydu

LOG_EVERY_SEC = 3.0

# OpenCV bazen CPU’yu şişirebiliyor, özellikle Nano’da gereksiz
try:
    cv2.setNumThreads(1)
except Exception:
    pass
...
def build_srt_writer_pipeline():
    ...
    # Isı için: maxperf-enable=1 KAPALI.
    # preset-level/bitrate makul. iframe aralığı da stabil.
    return (
        ...
        "control-rate=1 preset-level=1 maxperf-enable=0 ! "
        "h264parse config-interval=1 ! mpegtsmux alignment=7 ! queue leaky=downstream max-size-buffers=2 ! "
...
class TRTDetector(object):
    ...
    def _decode_cn_noobj(self, out_cn, orig_w, orig_h, ratio, dw, dh):
        # noobj: [x,y,w,h,cls1,cls2,...]  -> conf = max(cls_scores)
        ...
        cls_scores = out_cn[4:, :].astype(np.float32)
        cls_ids = np.argmax(cls_scores, axis=0).astype(np.int32)
        conf = np.max(cls_scores, axis=0).astype(np.float32)
    ...
    def decode_auto(self, out, orig_w, orig_h, ratio, dw, dh):
        ...
        # (C,N) bekliyoruz
        if r >= 6 and c > r:
            return self._decode_cn_noobj(out, orig_w, orig_h, ratio, dw, dh)

        # bazen transpose gelir
        out_t = out.T
        rt, ct = int(out_t.shape[0]), int(out_t.shape[1])
        if rt >= 6 and ct > rt:
            return self._decode_cn_noobj(out_t, orig_w, orig_h, ratio, dw, dh)
    ...
    while running:
        ...
        if run_infer:
            ...
            if now - last_log >= LOG_EVERY_SEC:
                print("[TRT] seq={} det={} infer_ms={:.1f}".format(seq, len(dets), infer_ms))
                last_log = now
        else:
            # inference olmayan framelerde de meta güncel kalsın
            with state_lock:
                latest_meta = {
                    "ts": time.time(),
                    "seq": int(seq),
...
def stream_loop():
    ...
    while running:
        ...
        # KUTU YOK: Jetson sadece ham frame’i stream ediyor
        frame = np.ascontiguousarray(frame)
        ...
        if now - t_log >= LOG_EVERY_SEC:
            with state_lock:
                detc = len(latest_dets)
                inf = float(latest_infer_ms)
            print("[SRT] sent={} det={} infer_ms={:.1f}".format(sent, detc, inf))
            t_log = now
...
def main():
    ...
    print("[SYS] SRT_LATENCY_MS:", SRT_LATENCY_MS, "INFER_EVERY_N:", INFER_EVERY_N)
